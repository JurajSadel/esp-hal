...
//! list of [`RegdmaLink`] nodes on PAU entry link 0 with no CPU involvement.
...
/// A single ADDR_MAP over the config registers, matching ESP-IDF.
...
/// they stay accessible at both backup and restore and this single ADDR_MAP is
/// all that is needed (as in ESP-IDF's `spi2_regs_retention`).
...
/// every time, so the PAU never walks a deregistered entry.
// Mirrors ESP-IDF's link setup + `pmu_sleep_enable_regdma_backup()`
// (active/sleep phases only).
