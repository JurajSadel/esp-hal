...
//! `cpu_retention`; there is no build logic here (it is shared with the C6 via
//! `retention::SysOp`). Region sizes note the sizing end register
//! (`count = ((end - base) / 4) + 1`).
//!
//! The H2 differs from the C6 only in register data: a shorter PCR run plus a
//! bus-clock update pulse, a different interrupt-matrix / HP-system / GPIO /
//! IO-MUX register count (fewer pins), and the SysTimer base. The SPI-mem,
//! TEE/APM, console-UART, SysTimer sequence and every CPU-domain base are
//! identical.
//!
//! References (ESP-IDF `v5.4`): `soc/esp32h2/system_retention_periph.c`,
//! `esp_hw_support/.../esp32h2/sleep_clock.c`, `.../esp32h2/sleep_cpu.c`.

use super::SysOp::{self, Continuous, Systimer, Uart, Wait, Write};
...
pub(crate) const OPS: &[SysOp] = &[
    ...
    Continuous { base: 0x6009_6000, count: 85 }, // PCR ..= PCR_PWDET_SAR_CLK_CONF_REG (+0x150)
    Continuous { base: 0x6009_6FF0, count: 1 },  // PCR_RESET_EVENT_BYPASS_REG
    Write { addr: 0x6009_6148, value: 0x1, mask: 0x1 }, // PCR_BUS_CLK_UPDATE (BUS_CLOCK_UPDATE)
    Wait { addr: 0x6009_6148, value: 0, mask: 0x1 },    // wait for it to self-clear
    ...
    Continuous { base: 0x6001_0000, count: 69 }, // INTMTX ..= INTMTX_CORE0_CLOCK_GATE_REG (+0x110)
    Continuous { base: 0x6009_5000, count: 12 }, // HP_SYSTEM ..= HP_SYSTEM_MEM_TEST_CONF_REG (+0x2c)
    // PRI_5: console UART0.
    Uart { base: 0x6000_0000 },
    // PRI_6: IO MUX + GPIO matrix (fewer pins than the C6).
    Continuous { base: 0x6009_0000, count: 29 }, // IO_MUX ..= IO_MUX_GPIO27_REG (+0x70)
    Continuous { base: 0x6009_1554, count: 32 }, // GPIO_FUNC0_OUT_SEL ..= GPIO_FUNC31_OUT_SEL
    Continuous { base: 0x6009_114C, count: 127 }, // GPIO_STATUS_NEXT ..= GPIO_FUNC124_IN_SEL
    Continuous { base: 0x6009_1000, count: 61 }, // GPIO ..= GPIO_PIN31_REG (+0xf0)
...
// consumed by `cpu_retention`. Identical to the C6 (same RISC-V core, cache and
// interrupt/timer controllers), but kept as chip data for clarity.
/// Interrupt priority controller (`INTPRI`).
pub(crate) const INTPRI_BASE: u32 = 0x600C_5000;
/// L1 cache control (`CACHE`).
